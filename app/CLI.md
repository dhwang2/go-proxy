...
gproxy network bbr disable --json
...
Completion covers subcommands and every argument whose values are a closed set: protocol types, configuration sources, managed services, core components, routing preset indexes, direct strategies, a firewall port's transport once its number is typed (`8443<Tab>` offers `8443/tcp`, `8443/udp`, `8443/both`), SS methods and TUIC congestion. Flags taking a free-form value, `--parameter` included, offer nothing rather than file names. Tab lists bare names with no descriptions, in every shell; `--help` says what each one does. Nothing gproxy takes is a file, so Tab never falls back to listing files: after a command with no arguments, such as `gproxy status`, and after the last argument a command takes, such as `gproxy log caddy-sub`, it offers nothing.
